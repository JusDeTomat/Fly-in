                ...
                if len(line.split(':')) > 1:
                    ...
                    if key == "nb_drones":
                            ...
                            if not no_parm:
                                ...
                                for element in parm:
                                    if element != '':
                                        ...
                                        if parmkey not in ["color", "max_drones", "zone"]:
                                            raise TypeError(f"{parmkey} is not a good metadata")
                                        if (parmkey == "color" and not isinstance(parmvalue,str)):
                                            raise TypeError(f"color parmeter need to be a str not '{parmvalue}'")
                                        try:
                                            if (parmkey == "max_drones" and not isinstance(parmvalue,int) and int(parmvalue) < 0):
                                                raise TypeError(f"max_drones parmeter need to be a positive int not '{parmvalue}'")
                                        except Exception:
                                            raise TypeError(f"max_drones parmeter need to be a positive int not '{parmvalue}'")
                                        if (parmkey == "zone" and not isinstance(parmvalue,str)):
                                            raise TypeError(f"zone parmeter need to be in the list ['normal','priority','restricted','blocked'] not '{parmvalue}'")
                                        if (parmkey == "zone"
                        ...
                        if len(mendatory_conn) == 2:
                            ...
                            for element in parm:
                                if element != '':
                                    parmkey, parmvalue = element.split('=')
                                    try:
                                        if (parmkey == "max_link_capacity" and not isinstance(parmvalue,int) and int(parmvalue) <= 0):
                                            raise TypeError(f"max_link_capacity parmeter need to be a positive int not '{parmvalue}'")
                                    except Exception:
                                        raise TypeError(f"max_link_capacity parmeter need to be a positive int not '{parmvalue}'")
                        ...
                        if dico_link in dico['link']:
                            raise TypeError(f"you can't have 2 time the same connection ({name})")
                        if dico_link_invers in dico['link']:
                            raise TypeError(f"you can't have 2 time the same connection even upside down ({name})")
